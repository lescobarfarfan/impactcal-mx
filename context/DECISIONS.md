...
- `CAL-WIND-02` [2026-06-11] Track interpolation timestep: **1 h default** (Eberenz hourly interpolation; CLIMADA tutorials 0.5–1 h), **conditional on a convergence test** (protocol in master doc §3.4; candidates Wilma 2005, Odile 2014, Patricia 2015, Willa 2018; criterion is convergence, never result magnitude — an observed finer-timestep→lower-losses effect is under investigation, `OQ-CAL-01`). Frozen identical in calibration and every downstream application: timestep bias is absorbed into `v_half`, so consistency dominates precision. [Eberenz2021]
