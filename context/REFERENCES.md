...
- **`[GumbelGloFAS-ref?]`** — *Gumbel distribution fit parameters for historical GloFAS river discharge data (1979–2023).* ETH Research Collection dataset, **hdl:20.500.11850/726304** (bitstream resolved via the DSpace API; MD5 `b18e8839e07acbc69ff8e8c26e0c7015` verified at freeze). This is the edition **frozen and used** (`CAL-RF-03`, decided 2026-07-23): it lives on the GloFAS **v4.0 0.05°** grid, matching the frozen discharge. The 1979–2015 edition that petals references (hdl:20.500.11850/641667) is on the v3 0.1° grid and is *unusable* here — `transform_ops.return_period` reindexes the fit onto the discharge coordinates with 1e-3° tolerance and `assert_no_fill_value`, so it raises. **Dataset authors/DOI to confirm.** → `OQ-CAL-08`
- **`[Leys2013-ref?]`** — Leys, C., Ley, C., Klein, O., Bernard, P., & Licata, L. (2013). *Detecting outliers: Do not use standard deviation around the mean, use absolute deviation around the median.* Journal of Experimental Social Psychology, 49(4), 764–766. **Confirm DOI/pages before manuscript.** Backs `CAL-GEN-14` (MAD robust z in `impactcal.inspeccion`). → `OQ-CAL-08`
